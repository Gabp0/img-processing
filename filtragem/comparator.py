...
def test_bilateral(img, level, sigma):
    """
    ...
    blur = cv2.bilateralFilter(noise_img, 9, sigma, sigma)

    psnr = cv2.PSNR(img, blur)
    print (f"Sigma = {sigma}, Noise level = {level}, PSNR = ", psnr)
...
def main(argv):
    if (len(sys.argv)!= 2):
        sys.exit("Use: comparator.py <imageIn>")
    ...
    kernel_range = [i for i in range(1, 51) if i % 2 == 1]

    test_filter("stacking.csv", img, range(1, 101), test_stacking, "nb_imgs")
    ...
    test_filter("bilateral.csv", img, range(10,201), test_bilateral, "sigmaValues")
